...
#takes in a dataframe of features
#returns a new dataframe of features scaled using min-max normalization
def minmax_normalize(X):
    
    #gets minimum and maximum values of each feature
    min_vals = X.min(axis='rows')
    max_vals = X.max(axis='rows')
    ...
    for feature in X.columns:
        ...
        for j in range(len(X)):
            row.append((X[feature][j] - minimum) / (maximum - minimum))
        scaled_X['scaled_' + feature] = row
        i += 1
...
def normal_equations(X, y):
    ...
    params_df = matrix1.dot(matrix2)
    
    #removes x0
    X.drop(['x0'], axis=1, inplace=True)
    
    #unpacks parameter dataframe into a list of parameters
    params = []
